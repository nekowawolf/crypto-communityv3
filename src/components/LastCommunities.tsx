...
export default function LastCommunities() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const fullUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL}/cryptocommunity`;
  ...
  const getPlatformIcon = (platform: string) => {
    ...
    if (p.includes('telegram')) return <FaTelegramPlane className="w-4 h-4" />;
    ...
    <>
      ...
      <div className="w-full mb-12 flex flex-col items-start text-left space-y-2">
        <h1 className="text-xl sm:text-2xl font-bold font-sans tracking-tight flex items-center">
          /last-added-communities
        </h1>

        <p className="text-fill-color/60 text-sm max-w-full sm:max-w-md leading-relaxed">
          Latest crypto communities added to the directory by contributors.{" "}
          <Link
            href="/add-community"
                  ...
                  >
                    {community.image_url ? (
                      ...
                      <div className="w-8 h-8 rounded-full flex items-center justify-center bg-blue-500/10 text-blue-400 border border-blue-500/20 shrink-0">
                        {getPlatformIcon(community.platforms)}
                      </div>
                    )}
                    ...
                    {community.added_by?.name && (
                      <div className="flex items-center gap-1.5 text-xs text-fill-color/60 font-mono mr-1.5">
                        ...
                        {community.added_by.url ? (
                          ...
                          <span className="font-semibold text-fill-color">{community.added_by.name}</span>
                        )}
