...
export default function LastCommits() {
  ...
  useEffect(() => {
    const loadCommits = async () => {
      try {
        const fullUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL}/githubrepo/commits/nekowawolf/crypto-communityv3-src?per_page=8`;
    ...
    <>
      ...
      <div className="w-full mt-24 mb-12 flex flex-col items-start text-left space-y-2">
        ...
        <p className="text-fill-color/60 text-sm max-w-full sm:max-w-md leading-relaxed">
          ...
          <a
            href="https://github.com/nekowawolf/crypto-communityv3-src"
            target="_blank"
            rel="noreferrer"
                    ...
                    <div key={commit.sha} className={`flex flex-row items-start justify-between p-4 ${idx !== (dateCommits as any[]).length - 1 ? 'border-b border-[var(--border-divider)]' : ''}`}>
                      ...
                      <div className="shrink-0 flex items-center gap-2">
                        <CommitMobileDropdown
                          ...
                          treeUrl={`https://github.com/nekowawolf/crypto-communityv3-src/tree/${commit.sha}`}
                        />
                        <div className="hidden sm:flex items-center font-mono text-xs border border-[var(--border-divider)] rounded-md overflow-hidden bg-transparent">
                          ...
                          <a href={`https://github.com/nekowawolf/crypto-communityv3-src/tree/${commit.sha}`} target="_blank" rel="noreferrer" className="px-2 py-1 text-fill-color/70 hover:text-fill-color hover:bg-[rgba(var(--fill-color-rgb),0.05)] transition-colors" title="Browse the repository at this point in the history">
          ...
          {commits.length > 4 && (
            <div className="mt-8 flex flex-col items-center gap-3">
              {!showMoreCommits ? (
                ...
                <>
                  <a
                    href="https://github.com/nekowawolf/crypto-communityv3-src/commits/main/"
                    target="_blank"
                    rel="noreferrer"
